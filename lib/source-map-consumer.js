...
import { readWasm } from "./read-wasm.js";
...
export class IndexedSourceMapConsumer extends SourceMapConsumer {
  ...
  originalPositionFor(aArgs) {
    ...
    const sectionIndex = binarySearch.search(
      ...
      function (aNeedle, section) {
        const cmp = aNeedle.generatedLine -
  ...
  generatedPositionFor(aArgs) {
    ...
    const nextSection = index >= 0 && index + 1 < this._sections.length
      ? this._sections[index + 1]
      : null;

    const generatedPosition = section &&
  ...
  eachMapping(aCallback, aContext, aOrder) {
    this._sections.forEach((section, index) => {
      const nextSection = index + 1 < this._sections.length
        ? this._sections[index + 1]
        : null;
